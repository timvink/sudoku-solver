...
def swordfish(p: 'Puzzle', digits: list[int] | None = None) -> bool:
    ...
    while update_found > 0:
        ...
        update_found_columns = _find_swordfish_in_lines(p, line_type="columns", digits=digits)
...
def _find_swordfish_in_lines(
    ...
    for num in digits or range(1, 10):
        ...
        cell_pairs = [cell_pair for cell_pair in cell_pairs if all(c in valid_candidate_cells for c in cell_pair)]
        
        # We need at least 3 lines to have a swordfish. Because 2 would be an X-wing.
        if len(cell_pairs) < 3:
            continue

        logging.debug(f"Looking for swordfish in {len(cell_pairs)} lines of type {line_type}. Considering pairs of '{num}'")
        ...
        while True:
            current_cell = _find_linked_cell(current_cell, cell_pairs, line_type=line_type)
            ...
            for cell_pair in cell_pairs:
                if current_cell in cell_pair:
                    current_cell = cell_pair[0] if cell_pair[1] == current_cell else cell_pair[1]
            ...
            for cell in cells_in_chain:
                ...
                for update_cell in update_cells:
                    if update_cell.remove_markup(num):
                        logging.debug(f"Swordfish in {line_type}: removed {num} from cell ({update_cell.row_id}, {update_cell.col_id})")
                        return True

    return False


def _find_linked_cell(cell: 'Cell', cell_pairs: list[list['Cell']], line_type: Literal["rows", "columns"]) -> Optional['Cell']:
    """
