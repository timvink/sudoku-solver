...
def _candidate_lines_iteration(blocks):
    ...
    for block in blocks:
        for value in range(1,10):
            ...
            possible_columns = list(set([c.column for c in block.cells if not c.is_solved and value in c.markup]))
            if len(possible_columns) == 1:
                    other_block_cells_in_column = [c for c in possible_columns[0].cells if not c.is_solved and c not in block.cells]
                    updated_cells = [c for c in other_block_cells_in_column if c.remove_markup(value)]
